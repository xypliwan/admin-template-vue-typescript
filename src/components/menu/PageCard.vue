  ...
  <div class="page-card">
    <div class="card-wrapper">
      <el-tabs :value="$route.meta.name" type="border-card" closable @tab-remove="removeTab" @tab-click="tabClick">
        <el-tab-pane v-for="(item,index) in tabList" :key="item.name" :label="item.title" :name="item.name"></el-tab-pane>
      </el-tabs>
    </div>
...
export default class extends Vue {
  get tabList() {
    return TabModule.tabList;
  }

  private handleClickItem(num: string) {
    //0当前  1其他  2全部
    let indexs = null;
    let item: TabConfig = {
    ...
    switch (num) {
      case '0': //0关闭当前
        ...
        break;
      case '1': //1关闭其他
        TabModule.delAllTabList();
        TabModule.addTabList(item);
        break;
      case '2': //关闭全部
        TabModule.delAllTabList();
        this.$router.push({
    ...
    this.tabList.forEach((el, i) => {
      if (el.name == targetName) {
        if (i !== 0 && this.$route.meta.name == targetName) {
          TabModule.delTabList({ start: i, end: 1 });
          this.$router.push({
  ...
  private tabClick(tab: RouteConfig) {
    this.$router.push({
      name: tab.name
    });
  }
